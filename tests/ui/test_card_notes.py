...
from _constants import (
    KEY_UP, KEY_DOWN, KEY_LEFT, KEY_RIGHT, KEY_OK, KEY_M1, KEY_M2, KEY_PWR,
    TAG_BTN_RIGHT,
)
...
# ----------------------------------------------------------------------
# Regression: ui.json-less BaseActivity plugins must survive PluginActivity
# ----------------------------------------------------------------------

class _DummyCustom(BaseActivity):
...
def test_framework_delegates_custom_activity_without_white_screen(env):
    bundle = {'plugin_dir': '', 'manifest': {'name': 'Dummy'},
              'ui_definition': None, 'entry_class': _DummyCustom,
              'translations': {}}
    actstack.start_activity(PluginActivity, bundle)
    top = actstack.get_current_activity()
    assert type(top).__name__ == '_DummyCustom'
